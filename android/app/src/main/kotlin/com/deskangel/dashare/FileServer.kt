 ...
 */
class FileServer( var context: Context, var uri: Uri, var shortName: String, var fileName: String?, host: String?, port: Int) : NanoHTTPD(host, port) {
        ...
        if (session?.uri == "/$shortName") {
            ...
            return when (val file = context.contentResolver.openInputStream(uri)) {
                null -> {
                    ...
                    response.addHeader("Content-Disposition", "attachment; filename=\"${fileName ?: shortName}\"")
